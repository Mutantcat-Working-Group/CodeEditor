...
export APP_NAME="CodeEditor"
export ASSETS_REPOSITORY="Mutantcat-Working-Group/CodeEditor"
export BINARY_NAME="codeeditor"
export CI_BUILD="no"
export GH_REPO_PATH="Mutantcat-Working-Group/CodeEditor"
export ORG_NAME="Mutantcat-Working-Group"
...
while getopts ":ilops" opt; do
  case "$opt" in
    i)
      export ASSETS_REPOSITORY="Mutantcat-Working-Group/CodeEditor-insiders"
      export BINARY_NAME="codeeditor-insiders"
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    ...
    if [[ -f "${HOME}/.gyp/include.gypi" ]]; then
      mv ~/.gyp/include.gypi ~/.gyp/include.gypi.pre-codeeditor
    else
      echo "{}" > ~/.gyp/include.gypi.pre-codeeditor
    fi
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    mv ~/.gyp/include.gypi.pre-codeeditor ~/.gyp/include.gypi
  fi
